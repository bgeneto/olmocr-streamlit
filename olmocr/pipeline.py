...
def get_server_url(endpoint=""):
    ...
    if VLLM_BASE_URL:
        return f"{VLLM_BASE_URL.rstrip('/')}{endpoint}"
    else:
        return f"http://localhost:{BASE_SERVER_PORT}{endpoint}"
...
async def process_page(args, worker_id: int, pdf_orig_path: str, pdf_local_path: str, page_num: int) -> PageResult:
    COMPLETION_URL = get_server_url("/v1/chat/completions")
...
async def main():
    ...
    if not args.vllm_base_url:  # Only check GPU if we're starting local vLLM server
        check_torch_gpu_available()
    ...
    if not args.vllm_base_url:
        ...
        logger.info(f"Using external vLLM server at: {args.vllm_base_url}")
